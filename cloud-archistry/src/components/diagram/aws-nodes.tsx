...
export const AWSCloudNode = memo(({ data, selected }: { data: BoundaryNodeData; selected?: boolean }) => {
  return (
    ...
    >
      ...
      <div className="absolute -top-3 left-4 flex items-center gap-1.5 bg-slate-800 px-2 py-0.5 border border-slate-600 rounded" style={{ pointerEvents: "auto", zIndex: 1000 }}>
...
export const RegionNode = memo(({ data, selected }: { data: BoundaryNodeData; selected?: boolean }) => {
  return (
    <div
      ...
      style={{ width: "100%", height: "100%", minWidth: 250, minHeight: 180, pointerEvents: "none", paddingTop: "20px" }}
    >
      <div style={{ pointerEvents: "auto" }}>
        <NodeResizer
          isVisible={selected}
      ...
      <div className="absolute -top-3 left-4 flex items-center gap-1.5 bg-blue-600 px-2 py-0.5 border border-blue-500 rounded" style={{ pointerEvents: "auto", zIndex: 1000 }}>
...
export const AvailabilityZoneNode = memo(({ data, selected }: { data: BoundaryNodeData; selected?: boolean }) => {
  return (
    ...
    >
      ...
      <div className="absolute -top-3 left-4 flex items-center gap-1.5 bg-sky-500 px-2 py-0.5 border border-sky-400 rounded" style={{ pointerEvents: "auto", zIndex: 1000 }}>
        <span className="text-sm">📍</span>
        <span className="text-white text-xs font-medium">{data.label}</span>
      </div>
      <Handle type="target" position={Position.Top} className={handleClass(selected)} />
