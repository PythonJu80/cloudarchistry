...
const SERVICE_TEMPLATES: Record<string, {
  ...
  "vpc": {
    create: (node, opts, ctx) => {
      ...
      const tags = opts.addTags ? ` --tag-specifications 'ResourceType=vpc,Tags=[{Key=Name,Value=${name}},{Key=Project,Value=${opts.projectName}},{Key=Environment,Value=${opts.environment}},{Key=ManagedBy,Value=CloudArchistry}]'` : "";
...
export function generateAwsCliCommands(
  ...
  const opts: GeneratorOptions = {
    ...
    projectName: options.projectName || "cloudarchistry",
